...
aplicado al deporte de motor y alta competicións},
...
\noindent Cabe aclarar que el índice de Calinski-Harabász depende del tamaño de la muestra, por lo que solo es comparable entre valores de $k$ de un mismo clasificador, no entre clasificadores distintos.
\subsection{Interpretación y limitaciones}
Los resultados han de leerse con honestidad. Los clasificadores de pilotaje y de circuito presentan coeficientes de Silhouette razonables (0,45 y 0,37) e índices de Davies-Bouldin bajos, indicando grupos bien definidos. La proyección de los circuitos lo confirma visualmente. El clasificador de tipo de vuelta, en cambio, presenta un Silhouette bajo (0,15). Lejos de ser un defecto del modelo, esto refleja la naturaleza de los datos.
El estilo de conducción es un \textit{continuo} y no un conjunto de categorías discretas por lo que no presenta unas fronteras nítidas, por lo que ningún agrupamiento puede separar grupos que en la realidad se solapan.
En datos reales de comportamiento, valores de Silhouette en el rango 0,1 a 0,3 son habituales, frente a los valores altos propios de conjuntos con grupos artificialmente separados.

\noindent Lo verdaderamente determinante es que, pese a ese solapamiento, la partición es muy estable. El ARI por validación cruzada alcanza 0,98 en tipo de vuelta y 0,93 en circuito, lo que demuestra que las asignaciones son reproducibles y no producto del azar.
El valor de los clasificadores reside en ofrecer una discretización estable e interpretable de un fenómeno continuo sirviendo como herramienta descriptiva en lugar de separar grupos perfectamente aislados.

\noindent Como limitación principal, la estabilidad temporal del clasificador de pilotaje es baja entre temporadas distintas. Esto es esperable, ya que la firma de un piloto evoluciona con los cambios de coche y reglamento de cada año. Una línea de trabajo futura sería entrenar firmas específicas por temporada o normalizar el efecto de la era reglamentaria.

...
import pandas as pd
import requests
from analisis_piloto import SpeedHeatMap as sm
...
from comparativas import Head2head as h2h
from comparativas import TeamTelemetry as tteam
from dinamica_vehicular import GGDiagram as gg
from dinamica_vehicular import Aero as aero
from dinamica_vehicular import TyreLoad as tload
...
def _back_to_grid_button():
    """Botón para volver a seleccionar otro circuito."""
    st.markdown("""
    <style>
...
def view_grid():
    ...
    st.markdown(
        ...
        '▸ Haz clic en un circuito para acceder al circuito</p>',
        unsafe_allow_html=True,
    )
...
def view_sessions():
    ...
    # Partimos el nombre: "Bahrain Grand Prix" → "BAHRAIN" + "GRAND PRIX"
...
def _hub_stats(f1s):
    ...
    m2.metric("Piloto con la vuelta rápida",        fast_drv)
    m3, m4 = st.columns(2)
    m3.metric("Número de pilotos",  len(f1s.drivers))
...
def _hub_podium(f1s):
    """Visualización de podio para el hub — podio cn colores de equipo"""
...
def _hub():
    """Hub de módulos — organizado por pestañas """
...
def view_analysis():
    ...
    # ── Botón volver al hub en todos laos ──
    _back_to_hub_button()
    ...
    if active == "Resumen de la sesión":
    ...
    elif active == "Resumen del Piloto":
        st.header("Resumen del Piloto")
        st.divider()
        ...
        if drv:
            dreport.render_driver_report(f1s, drv)

    elif active == "Mapa de Velocidad":
        st.header("Mapa de Calor · Velocidad (km/h)")
        st.divider()
        drv, lap_n = _driver_lap(f1s, "spd")
        spd_corners = st.toggle("Mostrar números de curva", value=show_corners, key="spd_corners")
        if lap_n:
            sm.render_speed_heatmap(f1s, drv, spd_corners, lap_n)

    elif active == "Mapa de Marchas":
        st.header("Mapa de Calor · Marchas (1-8)")
        st.divider()
        drv, lap_n = _driver_lap(f1s, "gear")
        gear_corners = st.toggle("Mostrar números de curva", value=show_corners, key="gear_corners")
        if lap_n:
            gm.render_gear_heatmap(f1s, drv, gear_corners, lap_n)

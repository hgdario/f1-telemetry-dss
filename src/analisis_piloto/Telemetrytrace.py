...
from LapTypeClassifier import render_lap_dna
from DriverStyleClassifier import render_driver_dna
from ui_assets import CSS_CIRCUIT_DNA
...
def _extract_lap_metrics(session: Session, driver: str, lap_number: int) -> dict:
    ...
    Retorna un diccionario con: lap_time, vmax, compound, tyre_life, gap_to_pole.
    """
    result = {
        "lap_time"    : None,
        "vmax"        : None,
...
def render_telemetry_trace(
    ...
    with tab_clásica:
        ...
        st.markdown(
            ...
            "TELEMETRY TRACE · DISTANCIA EN EL EJE X",
            unsafe_allow_html=True,
        )
        ...
        # CSS compartido (DNA line, stats, insight) — usado por las dos secciones siguientes
        st.markdown(CSS_CIRCUIT_DNA, unsafe_allow_html=True)

        # ---------------------------------------------------------
        # 7. TIPO DE VUELTA — K-means sobre 16k vueltas residualizadas por GP
        # ---------------------------------------------------------
        st.divider()
        st.caption("TIPO DE VUELTA · CLASIFICACIÓN CON K-MEANS")
        ...
        # ---------------------------------------------------------
        st.divider()
        st.caption("FIRMA DE PILOTAJE · ESTILO PERSONAL")

        # FastF1 puede devolver el código como número de coche ("1") o abreviatura ("VER").
        driver_abbr = driver
        try:
            info = session.get_driver(driver)
            if info is not None and "Abbreviation" in info.index:
                driver_abbr = str(info["Abbreviation"])
        except Exception:
            pass

        render_driver_dna(driver_abbr)
    with tab_simulacion:
